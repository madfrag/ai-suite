...
export default function ChatUI() {
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      ...
      setMessages(data.messages.filter((msg: Message) => msg.chat_session_id === chatSessionIdParam));
    };
  ...
  useEffect(() => {
    const grouped = allUserMessages.reduce((acc, msg) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4 h-screen flex flex-col text-foreground bg-background w-full">
      ...
      <Accordion type="single" collapsible className="mb-6 border border-border rounded-lg shadow-sm w-full" onValueChange={(value) => setIsOpen(!!value)}>
